...
| Script | What it does |
|--------|--------------|
| `fetch_option_chain.mjs` | Scrapes TradingView's Options Chain page (±10 strikes, nearest N expirations) for a set of tickers, filters to `ask <= $0.45`, and appends the results to the FATTAH detector's append-only contract history (`~/fattah/detector/contract_store.jsonl`, one line per candidate stamped with `discovered_at` + `run_id`), while also refreshing a derived "latest" snapshot (`~/fattah/detector/tracked_contracts.json`) in the shape `collect_multi_symbol.mjs` reads directly. See `skills/option-chain-navigate/SKILL.md` for the manual navigation path this automates, and its own file header for the DOM structure (`data-strike`, `data-cell-id`) and virtualized-table scroll-scrape it relies on. Run: `node scripts/fetch_option_chain.mjs [TICKER ...]` (defaults to all six FATTAH tickers). Flags: `--expirations=N` (default 4, nearest N dates) and `--concurrency=N` (default and only supported value: 1 — one CDP connection drives a single TradingView window, so ticker flows can't run in parallel against it). `contract_store.jsonl` has no rotation yet — at typical run volume it grows by low tens of MB/year, so manually archive it (e.g. rename to `contract_store-2026-Q3.jsonl`) if that ever becomes a concern. |
